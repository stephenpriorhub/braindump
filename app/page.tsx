      ...
      <div className={`max-w-[80%] ${isUser ? 'items-end' : 'items-start'} flex flex-col`}>
        ...
        {msg.stagedFiles && msg.stagedFiles.length > 0 && (
          <div className="w-full mt-1">
            {msg.stagedFiles.map((f, i) => <FileCard key={i} file={f} />)}
            <CommitPanel
...
export default function BrainDump() {
  ...
  const sendMessage = useCallback(async () => {
      ...
      while (true) {
        ...
        for (const line of lines) {
          ...
          if (data.type === 'text') {
            ...
            staged = data.files
            setMessages(prev => prev.map(m =>
              m.id === assistantId ? { ...m, stagedFiles: staged } : m
            ))
          }
        }
      }
    ...
    setLoading(false)
  }, [input, pendingImage, loading, messages, mode])
  ...
  return (
    <div className="flex flex-col h-screen" style={{ paddingTop: 44 }}>
      {/* Header */}
